...
for i, (x, y) in enumerate(coords):
    ax.scatter(x, y, s=90, color=colors[groups[i]], edgecolor="white", linewidth=0.8, zorder=3)
    ax.annotate(expr.columns[i], (x, y), textcoords="offset points", xytext=(6, 6), fontsize=8, color="#333")
ax.set_xlabel(f"PC1 ({pca.explained_variance_ratio_[0]*100:.1f}% var)")
ax.set_ylabel(f"PC2 ({pca.explained_variance_ratio_[1]*100:.1f}% var)")
ax.set_title("GSE71083: rat microglia, all probes", fontsize=10)
...
ns = ~(sig_up | sig_dn)
ax.scatter(res.log2FC[ns], res.neglog10p[ns], s=8, color="#bbb", alpha=0.6, linewidth=0, label="NS")
ax.scatter(res.log2FC[sig_up], res.neglog10p[sig_up], s=10, color="#c03030", alpha=0.7, linewidth=0, label="Up in fibrin")
ax.scatter(res.log2FC[sig_dn], res.neglog10p[sig_dn], s=10, color="#2255aa", alpha=0.7, linewidth=0, label="Down in fibrin")

lab = res[res.gene_symbol.isin(candidates)]
for _, row in lab.iterrows():
    ax.annotate(row.gene_symbol, (row.log2FC, row.neglog10p), fontsize=8,
                textcoords="offset points", xytext=(4, 3), color="#111")
    ax.scatter([row.log2FC], [row.neglog10p], s=24, color="black", zorder=4)

ax.axvline(0, color="#ddd", linewidth=0.8)
ax.set_xlabel("log2 fold change (fibrin vs. unstimulated)")
ax.set_ylabel("-log10(p-value)")
ax.set_title("GSE71083: fibrin-stimulated vs. unstimulated rat microglia", fontsize=10)
ax.legend(fontsize=8, frameon=False, loc="upper left")
ax.spines["top"].set_visible(False)
ax.spines["right"].set_visible(False)
fig.tight_layout()
fig.savefig("volcano.svg", format="svg")
plt.close(fig)
